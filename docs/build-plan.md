...
> **How this gets built:** the agent dev loop that produces these PRs — pipeline, objective gates, failure modes — is documented in [`loop-engineering.md`](loop-engineering.md), with the `/chromatik-mcp-loop` skill (`.claude/skills/chromatik-mcp-loop/`) as the operational checklist.
...
- [x] **PR-5d** — MIDI mapping tools — merged via [#105](https://github.com/oveddan/chromatik-mcp/pull/105) (read-side: `list_midi_devices`, `list_midi_mappings`, `list_midi_surfaces`) and [#106](https://github.com/oveddan/chromatik-mcp/pull/106) (mutations: `add_midi_mapping`, `remove_midi_mapping`, `set_midi_input`, `set_midi_surface_enabled`)
...
- [~] **PR-7c** — Catalog batches: stock LX modulators + Apotheneum — stock-LX half merged via [#67](https://github.com/oveddan/chromatik-mcp/pull/67) (26 `heronarts.lx.modulator`/`heronarts.lx.dmx` entries). The 37 `apotheneum.*` entries are bundled *here*, in this repo's own `package/src/main/resources/catalog/`; the slice as specified — generating them into the Apotheneum repo's `src/main/resources/catalog/` so that jar ships its own docs — is still open, sitting unmerged on Apotheneum's `catalog-patterns` branch
...
- [ ] *Follow-up (2026-07-20)* — Remove bundled `apotheneum.*` catalog entries once Apotheneum #38 is merged+released; remove `heronarts.lx.*` entries once LX #153 ships in the pinned LX release —
...
- [x] *Resolved (2026-06-10)* — `save_project` persistence tool deferred to **Phase 2**: v1 mutations stay in-memory; the user saves manually in Chromatik — **reversed (2026-07-25, [#154](https://github.com/oveddan/chromatik-mcp/issues/154))**: structure work done over MCP is unrecoverable on restart, and the `syncModelFile` write-through hazard needs a save-as escape hatch, not just visibility. `save_project`/`save_model` shipped — see the `save-project-tools` branch —
...
- **Formal verifier + objective `/goal` gate for the dev loop**: bind "done" to `mvn package` + headless harness + do→undo→assert green, checked by an independent verifier separate from the implementer (the maker/checker split). Today the review agent is ad hoc; formalize when the fan-out volume makes hand-spawning a reviewer per PR the bottleneck. *In progress:* the review half shipped via the `review-loop` branch (`/chromatik-mcp-review` + `docs/review-criteria.md` + a batched `simplify` pass); the objective-gate half (`pr-verifier`) already exists.
